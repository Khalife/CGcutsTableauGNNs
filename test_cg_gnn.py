...
############################################## Load graphs ###########################################################################
######################################################################################################################################
loader_ = torch.load(f"/local_workspace/khalsamm/{dataname}_{N}_{K}_{nb_samples}_graph_samples.pkl") 
...
GNNModel = AggCombGNN(dim_node_features, dim_edge_features, nb_iterations=5, final_out_dim = final_out_dim)
GNNModel = GNNModel.to(device)

GNNModel.load_state_dict(torch.load(f"./data/GNNModel_{dataname}_{nb_samples}_{dim_in}_{dim_out}_{num_epochs}_{batch_size}.torch_weights", weights_only=True))
GNNModel.eval()
